...
export default function BestWhateverEver() {
    ...
    useEffect(() => {
        Papa.parse("https://docs.google.com/spreadsheets/d/e/2PACX-1vTXRBc9CWHbb_L5Wp74nfCT2VhETeGtR3jKSXcUeyn101jzsEdwSYBgFSDrjh939sn2575Ywh1hITRB/pub?output=csv", {
            download: true,
            header: true,
            complete: (results) => {
    ...
    return (
        <div className="w-screen h-screen flex flex-col" style={{ fontFamily: "'Quicksand', sans-serif" }}>
            <Head>
                <title>Best Whatever Ever - {uid}</title>
                <link rel="icon" href="/favicon.ico" />
                <meta property="og:title" content="Best Whatever Ever" />
                <meta property="og:site_name" content="Vobble" />
                <meta property="og:url" content={'https://vobble.co/' + uid + '/best-whatever-ever'} />
                        ...
                        allowed ?
                            <div className="px-8 md:px-8 lg:px-24 py-24">
                                <div className="flex flex-col md:flex-row space-x-0 md:space-x-8 space-y-8 md:space-y-0">
                                    <div className="w-full h-fit md:w-1/2 p-4 bg-[#6667AB] flex flex-col space-y-4 rounded-lg">
                                        <img className="rounded-lg" src="/bestWhateverEver.jpeg" />
                                        {
                                            seriesTitle == "" || seriesDesc == "" ?
                                                loaderYellow :
                                                <>
                                                    <div className="text-2xl sm:text-4xl font-bold text-white" style={{ fontFamily: "'Gochi Hand', cursive" }}>{seriesTitle}</div>
                                                    <div className="text-lg font-regular text-white/50">{seriesDesc}</div>
                                                </>
                                        }
                                    </div>
