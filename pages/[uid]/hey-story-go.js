...
export default function HeyStoryGo() {
    ...
    useEffect(() => {
        Papa.parse("https://docs.google.com/spreadsheets/d/e/2PACX-1vTU1a5vMk8djHsRbuGyaCdBQhclfyvAJSvIuphyWt3SyCIzOm0UHdFlKbOAk4flxeREqEPkx4xRmB5L/pub?output=csv", {
            download: true,
            header: true,
            complete: (results) => {
    ...
    return (
        <div className="w-screen h-screen flex flex-col" style={{ fontFamily: "'Quicksand', sans-serif" }}>
            <Head>
                <title>Hey Story Go - {uid}</title>
                <link rel="icon" href="/favicon.ico" />
                <meta property="og:title" content="Hey Story Go" />
                <meta property="og:site_name" content="Vobble" />
                <meta property="og:url" content={'https://vobble.co/' + uid + '/hey-story-go'} />
                            ...
                            <div className="px-8 md:px-8 lg:px-24 py-24">
                                <div className="flex flex-col md:flex-row space-x-0 md:space-x-8 space-y-8 md:space-y-0">
                                    <div className="w-full h-fit md:w-1/2 p-4 bg-[#FFB100] flex flex-col space-y-4 rounded-lg">
                                        <img className="rounded-lg" src="/heyStoryGo.jpeg" />
                                        {
